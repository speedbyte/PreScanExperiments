...
% Warning
if logical(block.InputPort(7).Data) || logical(block.InputPort(8).Data)
    BorderColor = [1 0 0];
else
    ...
    switch message
        ...
        case 901
            if block.InputPort(8).data ~= 901;
                ...
                cla reset;
                [img, map, alpha] = imread('Models\B6','png');
        ...
        case 902
            if block.InputPort(8).data ~= 902;
                axes(V2x_sign);      % focus on slot SlotNum
                cla reset;
                [img, map, alpha] = imread('Models\B1','png');
                sizeAlpha = size(alpha);
                for i = 1 : sizeAlpha(1)
                    for j = 1 : sizeAlpha(2)
                        if alpha(i, j) == 0
        ...
        case 905
            if block.InputPort(8).data ~= 905;
                axes(V2x_sign);      % focus on slot SlotNum
                cla reset;
                [img, map, alpha] = imread('Models\J37','png');
                sizeAlpha = size(alpha);
                for i = 1 : sizeAlpha(1)
                    for j = 1 : sizeAlpha(2)
                        if alpha(i, j) == 0
        ...
        case 1030
            if block.InputPort(8).data ~= 1030;
                axes(V2x_sign);      % focus on slot SlotNum
                cla reset;
                [img, map, alpha] = imread('Models\A01030','png');
                sizeAlpha = size(alpha);
                for i = 1 : sizeAlpha(1)
                    for j = 1 : sizeAlpha(2)
                        if alpha(i, j) == 0
                ...
                imshow(img);
                axis image
                block.OutputPort(1).Data = 1030;
            end
        case 1040
            if block.InputPort(8).data ~= 1040;
                axes(V2x_sign);      % focus on slot SlotNum
                cla reset;
                [img, map, alpha] = imread('Pictures\A01040.png','png');
                sizeAlpha = size(alpha);
                for i = 1 : sizeAlpha(1)
                    for j = 1 : sizeAlpha(2)
                        if alpha(i, j) == 0
...
% Inputs
message  = block.InputPort(8).Data;
change   = block.InputPort(9).Data;

% Display lights
if change
    if message
        if block.InputPort(8).data ~= 801;
            axes(V2x_sign);      % focus on slot SlotNum
            cla reset;
